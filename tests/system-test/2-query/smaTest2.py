...
class TDTestCase:
    ...
    def run(self):
        # insert data
        dbname = "db1"
        tdSql.execute(f"create database {dbname} MAXROWS 200 MINROWS 50  STT_TRIGGER 1")
        tdLog.debug("create database {dbname} MAXROWS 200 MINROWS 50")
        self.create_tables("db1")
    ...
    def create_tables(self, dbname="db"):
        ...
        tdSql.execute(f"create table {dbname}.t4 using {dbname}.st tags(2)")
    ...
    def insert_data1(self, tbname, ts_start, count):
        ...
        for i in range(count):
            sql += " (%d,%d,%d)" % (ts_start + i*60000, i, i+1)
